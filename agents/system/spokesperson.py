...
class Spokesperson:
    ...
    def __init__(self, users_file: str = "data/users.yaml"):
        ...
        self._intent_cache_local: dict[str, dict] = {}
        self._cache_max: int = 500
        self._redis = self._init_redis()

    def _init_redis(self):
        """Return async Redis client if REDIS_URL is configured, else None."""
        ...
        try:
            ...
            logger.info(f"[Spokesperson] Redis intent cache enabled at {redis_url}")
            return client
        except Exception as e:
            logger.warning(f"[Spokesperson] Redis init failed — using local cache: {e}")
            return None

    async def _cache_get(self, key: str) -> Optional[dict]:
        if self._redis:
            try:
                val = await self._redis.get(f"rapid:intent:{key}")
                if val:
                    return json.loads(val)
            except Exception:
                pass
        return self._intent_cache_local.get(key)

    async def _cache_set(self, key: str, value: dict) -> None:
        if self._redis:
            try:
                await self._redis.set(
                    f"rapid:intent:{key}", json.dumps(value), ex=3600
                )
                return
            except Exception:
                pass
        if len(self._intent_cache_local) >= self._cache_max:
            oldest = next(iter(self._intent_cache_local))
            del self._intent_cache_local[oldest]
        self._intent_cache_local[key] = value

    # ── Authentication ────────────────────────────────────────────────────────

    def authenticate_user(self, user_id: str, credential: str) -> Optional[dict]:
        """Validate user credentials against PBKDF2 password hash."""
        user = self._users.get(user_id)
        if not user:
            logger.warning(f"Auth failed: unknown user '{user_id}'")
            return None
        if not verify_password(credential, user.get("password_hash", "")):
    ...
    async def classify_intent(self, query: str, user_profile: dict, history_context: str = "") -> dict:
        ...
        """
        cache_key = f"{query[:100]}|{user_profile.get('role', '')}|{history_context[:50]}"
        cached = await self._cache_get(cache_key)
        if cached:
            logger.debug(f"Spokesperson: intent cache hit for query='{query[:40]}'")
            return cached
    ...
    def reload_users(self):
        """Reload users from DB-backed store — called after any user store change."""
    ...
    def _load_users(self, users_file: str) -> dict:
        """Load users from DB (falls back to YAML on first run / test scenarios)."""
...
def _keyword_classify(query: str, permitted: list[str]) -> dict:
    ...
    Order of checks:
      1. Trivial patterns  (word-boundary match — prevents "hi" matching inside "machine")
      2. Dept keywords     (checked BEFORE general so company-specific queries like
                            "what is our Q3 salary budget?" route to Finance, not GENERAL)
      3. General patterns  (only if no dept keyword hit)
    """
    q = query.lower()

    # ── 1. Trivial check — use word-boundary regex ────────────────────────────
    # Plain substring matching causes false positives: "hi" in "machine" is True.
    # Word-boundary matching ensures "hi" only matches the standalone word "hi".
    for pat in _TRIVIAL_PATTERNS:
        if re.search(r"(?<!\w)" + re.escape(pat) + r"(?!\w)", q):
            return {"intent": INTENT_TRIVIAL, "dept_hints": [], "reason": "keyword: trivial pattern"}

    # ── 2. Dept keyword hits ──────────────────────────────────────────────────
    # Checked before general patterns so company-data queries win even when they
    # contain world-knowledge phrases ("what is our salary budget?" → finance).
    hits: dict[str, int] = {}
    ...
    # ── 3. General (world-knowledge) patterns ────────────────────────────────
    ...
    return {
        "intent": INTENT_SINGLE_DEPT,
        "dept_hints": permitted[:1] if permitted else [],
