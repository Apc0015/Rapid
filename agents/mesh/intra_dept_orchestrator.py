...
class IntraDeptOrchestrator:
    ...
    # ── Dynamic agent management (hot-reload, no restart needed) ─────────────

    def add_specialist(self, agent: "BaseEmployeeAgent") -> None:
        """
        Inject a new specialist into this orchestrator at runtime.
        If an agent with the same role_title already exists it is replaced.
        """
        # Remove existing agent with the same role_title if present
        self._specialists = [
            s for s in self._specialists if s.role_title != agent.role_title
        ]
        self._specialists.append(agent)
        self._by_role[agent.role_title] = agent
        logger.info(
            f"IntraDeptOrchestrator[{self._dept_tag}]: "
            f"added specialist '{agent.role_title}'"
        )

    def remove_specialist(self, role_title: str) -> bool:
        """
        Remove a specialist by role_title.
        Returns True if found and removed, False if not found.
        Built-in (hardcoded) agents can also be removed this way.
        """
        before = len(self._specialists)
        self._specialists = [s for s in self._specialists if s.role_title != role_title]
        self._by_role.pop(role_title, None)
        removed = len(self._specialists) < before
        if removed:
            logger.info(
                f"IntraDeptOrchestrator[{self._dept_tag}]: "
                f"removed specialist '{role_title}'"
            )
        else:
            logger.warning(
                f"IntraDeptOrchestrator[{self._dept_tag}]: "
                f"specialist '{role_title}' not found — nothing removed"
            )
        return removed
    ...
    def reload_custom_agents(self) -> int:
        ...
        remain untouched.
        Returns the number of custom agents now active.
        """
        from infrastructure.custom_agent_store import list_custom_agents
        from agents.base.dynamic_employee_agent import DynamicEmployeeAgent

        # Drop all existing dynamic agents for this dept
        self._specialists = [
            s for s in self._specialists
            if not isinstance(s, DynamicEmployeeAgent)
        ]
        ...
        logger.info(
            f"IntraDeptOrchestrator[{self._dept_tag}]: "
            f"reloaded {len(records)} custom agent(s)"
        )
        return len(records)
