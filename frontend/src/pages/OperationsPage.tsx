import { ArrowRightLeft, Database, Play, Plus, RefreshCw, Search, ShieldCheck, TimerReset, Workflow } from 'lucide-react';
...
export function OperationsPage() {
  ...
  async function ingest(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      } else await apiRequest(`/organization/data/sources/${ingestSource.id}/documents`, { method: 'POST', body: JSON.stringify({ name: form.get('name'), content }) });
      setIngestSource(null); await loadDepartment(activeDepartment); notify('Source data ingested and scoped.');
  ...
  return (
    <AdminShell title="Organization operations" description="Run governed department playbooks, inspect evidence, and control integrations from one console.">
      ...
      <Modal open={sourceDialog} title="Add governed source" context="Data source" size="small" onClose={() => setSourceDialog(false)}><form className="stack-form" onSubmit={createSource}><label className="admin-label">Source name<input name="name" maxLength={160} required placeholder="People handbook" /></label><label className="admin-label">Data type<select name="source_type" defaultValue="unstructured"><option value="unstructured">Documents and knowledge</option><option value="structured">Structured records</option></select></label><label className="admin-label">Connector type<select name="connector_type" defaultValue="manual"><option value="manual">Manual / sandbox</option><option value="database">Database connector</option><option value="storage">Drive or storage connector</option><option value="api">Application API connector</option></select></label><label className="admin-label">Classification<select name="classification" defaultValue="internal"><option value="internal">Internal</option><option value="confidential">Confidential</option><option value="restricted">Restricted</option></select></label><button className="product-button primary" type="submit">Add source</button></form></Modal>
      <Modal open={Boolean(ingestSource)} title={ingestSource?.source_type === 'structured' ? `Add ${ingestSource?.name} records` : `Add ${ingestSource?.name} document`} context="Source ingestion" onClose={() => setIngestSource(null)}><form className="stack-form" onSubmit={ingest}>{ingestSource?.source_type !== 'structured' ? <label className="admin-label">Document name<input name="name" maxLength={255} required placeholder="Leave policy" /></label> : null}<label className="admin-label">{ingestSource?.source_type === 'structured' ? 'Records (JSON array)' : 'Document content'}<textarea name="content" maxLength={2_000_000} rows={10} required placeholder={ingestSource?.source_type === 'structured' ? '[{"account":"Acme","stage":"qualified"}]' : 'Paste an approved policy or knowledge document.'} /></label><button className="product-button primary" type="submit">Ingest data</button></form></Modal>
