import { ArrowUpRight, BookOpen, CalendarCheck2, CircleCheck, Download, FileCheck2, FileText, LockKeyhole, Pencil, Plus, RefreshCw, Search as SearchIcon, Sparkles, UsersRound } from 'lucide-react';
...
import { RapidChat } from '../intelligence/RapidChat';
...
export function ActionRow({ action, onChange, editable = capabilitiesFor(getProfile()).operateDepartment }: { action: ActionItem; onChange: (id: string, status: string) => Promise<void>; editable?: boolean }) {
  return (
    <article className="action-row">
      ...
      <div className="action-controls">
        ...
        {editable ? <select data-action={action.id} aria-label={`Status for ${action.title}`} value={action.status} onChange={(event) => void onChange(action.id, event.target.value)}>
          <option value="open">Open</option><option value="in_progress">In progress</option><option value="done">Done</option>
        </select> : <StatusTag value={action.status} />}
      </div>
    </article>
  );
}
...
export function OverviewView({ data, navigate, openMeeting, changeAction, openDepartmentReport }: WorkspaceViewProps) {
  ...
  const companyView = capabilitiesFor(getProfile()).configureTenant;
  const canUpdateActions = capabilitiesFor(getProfile()).operateDepartment;
  ...
  return (
    <section className="portal-view active" data-portal-view="overview">
      <div className="view-role-context"><p className="section-context">{companyView ? 'Company pulse' : 'Department control center'}</p><p>{companyView ? 'Company-wide outcomes, risks, and decisions across every enabled team.' : 'Your department’s work, risks, meetings, and commitments.'}</p></div>
      <section className="metric-strip overview-metrics" aria-label="Organization snapshot">
        <article><span>{companyView ? 'People' : 'Team records'}</span><strong id="metric-employees">{overview.metrics.employees}</strong><small>{companyView ? 'Active organization records' : 'Visible department records'}</small></article>
        <article><span>{companyView ? 'Operating teams' : 'Work areas'}</span><strong id="metric-departments">{overview.metrics.departments}</strong><small>{companyView ? 'Focused work areas enabled' : 'Departments in your scope'}</small></article>
        <article><span>Open actions</span><strong id="metric-actions">{overview.metrics.open_actions}</strong><small>{companyView ? 'Cross-team commitments' : 'Commitments in your scope'}</small></article>
      ...
      <section className="workspace-section overview-departments"><div className="section-title"><div><p className="section-context">Operating map</p><h2>Work area health</h2><p>Focused work areas, visible owners, and a shared review process.</p></div><button className="text-button" type="button" onClick={() => navigate('departments')}>Open work areas</button></div><div id="overview-departments" className="department-grid">{overview.departments.map((department) => <button className="department-summary" data-report-department={department.key} key={department.key} type="button" onClick={() => openDepartmentReport(department.key)}><span className={`signal-dot ${department.status}`} /><div><strong>{department.name}</strong><small>{department.lead}</small></div><b>{department.open_actions} open</b></button>)}</div></section>
...
export function MeetingsView({ data, openMeeting }: WorkspaceViewProps) {
  ...
  const upcoming = data.meetings.filter((meeting) => meeting.status === 'scheduled');
  const decisions = data.meetings.reduce((total, meeting) => total + meeting.decisions.length, 0);
  const followUps = data.meetings.reduce((total, meeting) => total + (meeting.actions?.filter((action) => action.status !== 'done').length ?? 0), 0);
  return <section className="portal-view active meetings-page" data-portal-view="meetings"><section className="meetings-summary"><div><p className="section-context">Operating cadence</p><h2>Keep every review connected to a decision and an owner.</h2><p>Open any meeting to prepare the agenda, record notes, capture decisions, invite participants, and assign the follow-through.</p></div><div className="meetings-summary-metrics"><article><CalendarCheck2 size={16} /><strong>{upcoming.length}</strong><span>upcoming</span></article><article><FileCheck2 size={16} /><strong>{decisions}</strong><span>decisions</span></article><article><CircleCheck size={16} /><strong>{followUps}</strong><span>open follow-ups</span></article></div></section><div className="toolbar"><SegmentedControl id="meeting-filter" value={status} onChange={setStatus} options={[[ 'all', 'All' ], [ 'scheduled', 'Upcoming' ], [ 'completed', 'Completed' ]]} /><span id="meeting-total" className="result-count">{countLabel(meetings.length, 'meeting')}</span></div><section className="workspace-section meetings-list-section"><div className="section-title"><div><h2>{status === 'completed' ? 'Completed reviews' : status === 'scheduled' ? 'Upcoming reviews' : 'Meeting cadence'}</h2><p>Choose a meeting to prepare the discussion or update its record.</p></div></div><div id="meetings-list"><MeetingList meetings={meetings} onOpen={openMeeting} expanded /></div></section></section>;
}
...
function ProjectIntelligence({ projects, onRefresh }: { projects: RegisteredProject[]; onRefresh: () => Promise<void> }) {
  ...
  const isTenantAdmin = capabilitiesFor(profile).configureTenant;
  const canManageSelectedProject = isTenantAdmin || ['owner', 'manager'].includes(selected?.member_role ?? '');
  const scopedDepartments = permittedDepartments(profile, Object.keys(DEPARTMENTS));
  const canReviewAction = (action: AgentAction) => isTenantAdmin || (capabilitiesFor(profile).operateDepartment && scopedDepartments.includes(action.agent_dept));
...
export function ChatView({ data }: WorkspaceViewProps) {
  return <RapidChat data={data} />;
}

export function ReportsView({ data, reportDepartment, setReportDepartment, reportSignal }: WorkspaceViewProps) {
  const reportDepartments = permittedDepartments(getProfile(), data.overview.departments.map((department) => department.key));
  const [report, setReport] = useState<OperatingReport | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  useEffect(() => {
    const defaultDepartment = reportDepartments[0];
    if (defaultDepartment && !reportDepartments.includes(reportDepartment)) setReportDepartment(defaultDepartment);
  }, [reportDepartment, reportDepartments, setReportDepartment]);
...
export function SettingsView({ data }: WorkspaceViewProps) {
  ...
  const canConfigureTenant = capabilitiesFor(profile).configureTenant;
  ...
  const trust = data.configuration?.trust_summary;
  return <section className="portal-view active" data-portal-view="settings"><div className="settings-grid"><section className="workspace-section"><div className="section-title"><div><h2>Account</h2><p>Your current workspace identity.</p></div></div><dl id="account-details" className="detail-list">{account.map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{String(value).replaceAll('_', ' ')}</dd></div>)}</dl></section>{canConfigureTenant ? <><section className="workspace-section"><div className="section-title"><div><h2>AI runtime</h2><p>The runtime selected for this workspace.</p></div></div><div id="settings-models" className="connection-list">{data.configuration ? data.configuration.models.map((model) => <article className="connection-row" key={model.provider}><div><strong>{model.provider}</strong><small>{model.model_name || 'No model selected'} · {model.endpoint || 'No endpoint'}</small></div><StatusTag value={model.enabled ? 'enabled' : 'disabled'} /></article>) : <EmptyState>Runtime configuration is unavailable.</EmptyState>}</div></section>{trust ? <section className="workspace-section settings-wide"><div className="section-title"><div><h2>Workspace safeguards</h2><p>What RAPID can access, explain, and act on in this workspace.</p></div><Link className="text-button" to="/admin/configuration">Review controls <ArrowUpRight size={12} /></Link></div><div className="trust-control-grid compact">{[trust.boundary, trust.runtime, trust.connections, trust.evidence, trust.approvals].map((control, index) => <article className="trust-control" key={control.title}><span className={`trust-status ${control.status}`}>{index < 3 ? <LockKeyhole size={13} /> : <FileCheck2 size={13} />}{control.status.replaceAll('_', ' ')}</span><strong>{control.title}</strong><p>{control.detail}</p></article>)}</div></section> : null}<section className="workspace-section settings-wide"><div className="section-title"><div><h2>Organization connections</h2><p>SSO, databases, collaboration, and knowledge storage.</p></div><Link className="text-button" to="/admin/integrations">Manage integrations <ArrowUpRight size={12} /></Link></div><div id="settings-connections" className="connection-list">{data.configuration ? data.configuration.connections.map((connection) => <article className="connection-row" key={connection.connection_key}><div><strong>{connection.label}</strong><small>{connection.kind} · {connection.configuration.provider ?? connection.configuration.engine ?? 'not configured'}</small></div><StatusTag value={connection.status} /></article>) : <EmptyState>Connection configuration is unavailable.</EmptyState>}</div></section><section className="workspace-section settings-wide"><div className="section-title"><div><h2>Administration</h2><p>Manage access, teams, and service configuration.</p></div></div><div className="settings-actions"><Link className="product-button secondary" to="/admin/users">Users and invitations</Link><Link className="product-button secondary" to="/admin/integrations">Startup integrations</Link><Link className="product-button secondary" to="/admin/configuration">Tenant configuration</Link><Link className="product-button secondary" to="/operations">Department operations</Link></div></section></> : <section className="workspace-section settings-wide"><div className="section-title"><div><h2>Access</h2><p>Your role controls the departments, projects, and sources available in this workspace.</p></div></div><EmptyState>Tenant configuration is managed by your organization administrator.</EmptyState></section>}<section className="workspace-section settings-wide"><div className="section-title"><div><h2>Platform readiness</h2><p>API dependencies and durable background processing.</p></div></div><div id="settings-runtime" className="runtime-grid"><article><span>API</span><strong>{data.readiness?.status ?? 'unavailable'}</strong></article>{Object.entries(checks).map(([name, check]) => <article key={name}><span>{name.replaceAll('_', ' ')}</span><strong>{check.status}</strong></article>)}<article><span>Active workers</span><strong className={workers?.active_count ? '' : 'text-error'}>{workers?.active_count ?? 0}</strong></article><article><span>Queued jobs</span><strong>{jobStats.queued ?? 0}</strong></article><article><span>Retries</span><strong>{jobStats.retry ?? 0}</strong></article><article><span>Dead letters</span><strong className={jobStats.dead_letter ? 'text-error' : ''}>{jobStats.dead_letter ?? 0}</strong></article></div></section></div></section>;
}
