import { ArrowLeft, Building2, Link2, Menu, Network, Settings2, UsersRound, X } from 'lucide-react';
  ...
  return (
    <div className="product-page product-shell portal-shell admin-shell">
      <aside className={`product-sidebar portal-sidebar${navigationOpen ? ' open' : ''}`}>
        ...
        <div className="workspace-switcher"><span>NL</span><div><strong>Northstar Labs</strong><small>{capabilities.configureTenant ? 'Administration' : 'Department operations'}</small></div></div>
        <nav className="product-nav portal-nav" aria-label="Administration navigation">
          ...
          <p className="nav-group-label">{capabilities.configureTenant ? 'Administration' : 'Operations'}</p>
          {capabilities.configureTenant ? <><Link className={location.pathname === '/admin/configuration' ? 'active' : ''} to="/admin/configuration" onClick={closeNavigation}><span><Settings2 size={13} /></span>Configuration</Link><Link className={location.pathname === '/admin/integrations' ? 'active' : ''} to="/admin/integrations" onClick={closeNavigation}><span><Link2 size={13} /></span>Integrations</Link><Link className={location.pathname === '/admin/users' ? 'active' : ''} to="/admin/users" onClick={closeNavigation}><span><UsersRound size={13} /></span>Users and access</Link></> : null}
          {capabilities.operateDepartment ? <Link className={location.pathname === '/operations' ? 'active' : ''} to="/operations" onClick={closeNavigation}><span><Network size={13} /></span>Department operations</Link> : null}
        </nav>
      ...
      <main className="product-main portal-main">
        ...
        <header className="workspace-header portal-header"><div><p className="auth-kicker">{capabilities.configureTenant ? 'Tenant administration' : 'Department operations'}</p><h1>{title}</h1><p>{description}</p></div><span className="sandbox-badge">Synthetic demo</span></header>
        {children}
      </main>
