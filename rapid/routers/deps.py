...
def _load_users() -> dict:
    ...
    p = Path("data/users.yaml")
    return _yaml.safe_load(p.read_text()) if p.exists() else {}
...
def auth_user_password(user_id: str, password: str) -> dict:
    ...
    ok = verify_password(password, stored) if user.get("password_hash") else (password == stored)
    if not ok:
        raise HTTPException(status_code=401, detail="Authentication failed")
    return user
