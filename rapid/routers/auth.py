...
async def login(body: LoginRequest):
    ...
    users = _yaml.safe_load(users_path.read_text()) or {}
    user  = users.get(body.user_id)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid credentials")

    stored = user.get("password_hash") or user.get("token", "")
    ok = verify_password(body.password, stored) if user.get("password_hash") else (body.password == stored)
    if not ok:
        raise HTTPException(status_code=401, detail="Invalid credentials")
...
async def refresh_token(body: RefreshRequest):
    ...
    # Reload user to get latest role + permissions
    import yaml as _yaml
    users_path = Path("data/users.yaml")
    users = _yaml.safe_load(users_path.read_text()) or {}
...
async def change_password_endpoint(body: ChangePasswordRequest, current_user: dict = Depends(get_current_user)):
    ...
    import yaml as _yaml
    users = _yaml.safe_load(Path("data/users.yaml").read_text()) or {}
    user  = users.get(user_id, {})
    stored = user.get("password_hash") or user.get("token", "")
    ok = verify_password(body.current_password, stored) if user.get("password_hash") else (body.current_password == stored)
    if not ok:
