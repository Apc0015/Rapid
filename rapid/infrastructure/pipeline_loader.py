...
class UnstructuredPipeline:
    ...
    async def _execute(self, query: str, user_role: str) -> UnstructuredResult:
        ...
        use_hyde  = retriever_cfg.get("use_hyde", False)

        if use_hyde:
            rewritten_query = await self._hyde(rewritten_query)
    ...
    async def _hyde(self, query: str) -> str:
        ...
        llm    = get_llm()
        prompt = (
            f"Write a brief hypothetical answer to this question as if you "
            f"had access to all relevant documents:\n\nQuestion: {query}"
        )
        try:
            return await llm.chat(prompt, max_tokens=150)
        except Exception:
            return query
